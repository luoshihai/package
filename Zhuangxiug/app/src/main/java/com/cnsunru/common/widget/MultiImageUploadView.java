...
public class MultiImageUploadView extends FlowLayout {
    ...
    int numCol = 3;// 图片列数
    int max = 9;
    int closeRes = com.sunrun.sunrunframwork.R.drawable.ic_del;// 删除按钮的资源
    ...
    public void setAddHandlerImage(int res) {
        ((ImageView) handlerView.findViewById(com.sunrun.sunrunframwork.R.id.handlerView)).setImageResource(res);
    }
    ...
    void fileCompress(List<File> compress_files, File file, int quality) {
        ...
        if (!imgPath.startsWith("http") && quality != 100) {
            String aimPath = new File(IMG_CACHE_DIR, file.getName())
                    .getAbsolutePath();
            String compress_path = UIUtils.saveBitmapToFile(imgPath,
...
//                tvValues.append("方法三: height:"+height + ",width:" + width + "..\n");
        ...
        ImageView handlerView = (ImageView) findViewById(com.sunrun.sunrunframwork.R.id.handlerView);
        removeAllViews();
        if (handlerView == null) {
            handlerView = new ImageView(getContext());
            handlerView.setId(com.sunrun.sunrunframwork.R.id.handlerView);
            handlerView.setScaleType(ScaleType.FIT_XY);
            handlerView.setImageResource(com.sunrun.sunrunframwork.R.drawable.ic_add_img);
        }
    ...
    public static class SquareImageSizeConfig extends ImageSizeConfig {
        ...
        public int getWidth() {
            ...
            MarginLayoutParams layoutParams= (MarginLayoutParams) uploadView.getLayoutParams();
//            int uploadViewWidth
//            uploadView.getWidth();
