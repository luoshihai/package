...
//    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        TableColumn column = (TableColumn) getChildAt(0);
        float y = 0;
        for (int i = 1; i < maxRowCount; i++) {
            int height = column.getItemHeight(i-1);
//            float y = i * tableRowHeight;
    ...
    private void useAdapter() {
        ...
        for (int i = 0; i < count; i++) {
            final int col = i;
            TableColumn tableColumn = new TableColumn(getContext(), adapter.getColumnContent(i), this, new TableColumn.ItemViewIntercept() {
                @Override
                public void onInterceptItemView(TextView item, int row) {
                    adapter.onInterceptItemView(item, row, col);
                }
            });
            addView(tableColumn);
        }
